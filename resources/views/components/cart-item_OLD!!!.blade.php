...
<div class="flex items-center w-full h-[112px] py-[16px] border-b border-dashed border-gray-200">
    ...
    <!-- Чекбокс привязан к глобальной форме cart-form атрибутом form -->
    <div class="w-[30px]">
    ...
    <div class="w-[540px] pr-[40px] flex flex-col justify-center">
        ...
        {{-- Кнопка удаления привязана к cart-form через атрибут form --}}
        <button type="submit" 
                form="cart-form"
                name="action" 
                value="document.getElementById('cart-action').remove_{{ $item->id }}"
    ...
    <div class="flex-1">
        {{-- 
           Инпут привязан к cart-form. 
           При изменении мы выставляем action='update' и отправляем ГЛОБАЛЬНУЮ форму.
        --}}
        <input type="number" name="quantities[{{ $item->id }}]" min="1" max="{{ $isAvailable ? $available : 1 }}"
            value="{{ $item->quantity }}"
            form="cart-form" 
            onchange="document.getElementById('cart-action').value='update'; document.getElementById('cart-form').submit()"
