{{-- Фильтр-радио: одиночный выбор.
     Auto-submit через filterCheckboxChange() — та же функция что и для checkbox
...
@php
    $activeValue = is_array($active) ? ($active[0] ?? null) : $active;

    // Считаем доступные опции (products_count > 0).
    $availableOptions = $property->options->filter(fn($o) => ($o->products_count ?? 0) > 0);
